...
class App {
    ...
    #setupModel() {
        ...
        gltfLoader.load("data/gltf/model/scene.gltf", (gltf) => {
            ...
            obj3d.traverse(node => {
                ...
                if (node instanceof THREE.Mesh) {
                    if (node.material) {
                        //부위별 체크해서 찾기. 99머리카락 93얼굴
                        if (node.name === "Object_99") {
                            // node.material = new THREE.MeshBasicMaterial({ color: "red" });
                            node.material.transparent = true;
                        }
                        if (node.name === "Object_93") {
                            // node.material = new THREE.MeshBasicMaterial({ color: "red" });
            ...
            obj3d.position.set(0, -0.7, 0);
            obj3d.scale.set(0.7, 0.7, 0.7);
